...
document.addEventListener('DOMContentLoaded', () => {
        ...
        window.Echo.channel(`room.${roomId}`)
            .listen('.QuestionUpdated', (e) => {
                ...
                if (quizStarted) {
                    ...
                    countdownDisplayEl.innerText = "Starting in 5 seconds...";
    ...
    function updateParticipantsAnswers(submittedAnswers) {
        ...
        if (submittedAnswers.length === 0) {
            const noAnswersLi = document.createElement('li');
            noAnswersLi.textContent = 'No answers submitted yet.';
            answersContainer.appendChild(noAnswersLi);
            return;
        }

        submittedAnswers.forEach(answer => {
            const li = document.createElement('li');
            li.textContent = `${answer.participant.name}: ${answer.sub_answer} (${answer.correct ? 'Correct' : 'Incorrect'})`;
